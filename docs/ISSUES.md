...
| #026 | Add sync range by Book | P2 | Datasource | Implemented: `bookstack_book` supports Book-scoped sync with page-level output, shared-client traversal support, mocked tests, docs updates, successful repackaging, Docker API reachability checks, successful Dify console sign-in verification, and installed-runtime datasource registration inside Dify. Content-level Book smoke after credential authorization remains a follow-up validation item. |
| #027 | Add sync range by Chapter | P2 | Datasource | Implemented: `bookstack_chapter` supports Chapter-scoped sync with page-level output, mocked tests, docs updates, successful repackaging, Docker API reachability checks, successful Dify console sign-in verification, and installed-runtime datasource registration inside Dify. Content-level Chapter smoke after credential authorization remains a follow-up validation item. |
| #028 | Add metadata mapping for Knowledge Pipeline | P2 | Data Mapping | Implemented: Datasource metadata now includes stable source/scope/id/title/URL/tags/timestamp/content-format fields across Page/Chapter/Book outputs, with automated regression, successful repackaging, Docker API reachability checks, successful Dify console sign-in verification, and installed-runtime declaration validation for nullable fields and list-form tags. Content-level runtime smoke after credential authorization remains a follow-up validation item. |
| #029 | Prepare Marketplace submission | P2 | Release | Planned: release package and submission checklist will be prepared for Marketplace PR after the broader issue sequence is complete. |
